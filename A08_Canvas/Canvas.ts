window.onload = function main(): void {
    ...
    for(let a : number = 0; a < 2; a++){
        ...
        for(let c : number = 0; c < randomNumber(10); c++)
    ...
    for(let b : number = 0; b < randomNumber(500, 20); b++){
        let d : number = randomNumber(3);
        crc2.beginPath();
        if(d == 1)
            crc2.arc(randomNumber(950), randomNumber(1200), randomNumber(50), randomNumber(365), randomKreis(2) * Math.PI);
        
        else if(d == 2){
            let e : number = randomNumber(950);
            let f : number = randomNumber(1200);
            crc2.moveTo(e, f);
...
function randomKreis(max : number) : number{
